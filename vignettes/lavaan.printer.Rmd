...
This article illustrate how to use
the two main functions of `lavaan.printer`:
`parameterEstimates_table_list()` and
...
- `parameterEstimates_table_list()`:
  ...
  organized based on the output for
  parameter estimates in the `summary()`
  output of `lavaan`. The data frames
  can be customized in a lot of ways,
  such as adding or deleting columns
  created by user-supplied functions or
...
- `print_parameterEstimates_table_list()`:
  ...
  Most of the customization is done when
  calling `parameterEstimates_table_list()`.

# Scenarios

## Just Create a List of Data frames

In the simplest case, `parameterEstimates_table_list()`
can bee used to generate the usual parameter
estimates results of `lavaan`, but as
a list of data frames. Arguments for
...
the missing data handling method. The
